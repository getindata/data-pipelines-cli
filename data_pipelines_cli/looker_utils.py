...
def _prepare_repo_changes(src: pathlib.Path, dest: pathlib.Path) -> None:
    ...
    if local_repo_gen_path.exists():
        ...
        if local_repo_gen_path.joinpath(views_subdir).exists():
            rmtree(local_repo_gen_path.joinpath(views_subdir))

    _copy_all_files_by_extention(src, local_repo_gen_path, "model.lkml")
    _copy_all_files_by_extention(src, local_repo_gen_path.joinpath(views_subdir), "view.lkml")

    with open(f"{local_repo_gen_path}/readme.txt", "w") as readme:
        readme.write(
            """models and vievs catalogs are generated by data-pipelines-cli. Do not edit manually!
        If you need to add something manually, put your changes in the other folder.
        """
        )
